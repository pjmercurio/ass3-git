...
void BezierPatch::adaptive_subdivision(float error) {
    ...
    for(unsigned int i=0; i<p.size(); i++) {
        for(unsigned int j=0; j<p[i].size(); j++) {
            ...
            Vector3f *a1 = calculateUVpoint(i*step_size, j*step_size);
            Vector3f *b1 = calculateUVpoint((i+1)*step_size, j*step_size);
            Vector3f *c1 = calculateUVpoint(i*step_size, (j+1)*step_size);
            Vector3f *d1 = calculateUVpoint((i+1)*step_size, (j+1)*step_size);
...
void BezierPatch::subdivide_triangle(vector<Vector3f *>verts, vector<Vector2f *>uv_verts, float err_margin) {
    ...
    Vector3f *u1 = calculateUVpoint((*uv1)[0], (*uv1)[1]);
    // midpoint between vert0 and vert1
    Vector3f v1_ = (*verts[0] + *verts[1])/2.0f;
    Vector3f *v1 = new Vector3f(v1_[0], v1_[1], v1_[2]);
    ...
    Vector3f *u2 = calculateUVpoint((*uv2)[0], (*uv2)[1]);
    // midpoint between vert1 and vert2
    Vector3f v2_ = (*verts[1] + *verts[2])/2;
    Vector3f *v2 = new Vector3f(v2_[0], v2_[1], v2_[2]);
    // if the magnitude is less than the margin of error, it passes
    ...
    Vector3f *u3 = calculateUVpoint((*uv3)[0], (*uv3)[1]);
    // midpoint between vert2 and vert0
    Vector3f v3_ = (*verts[2] + *verts[0])/2;
    Vector3f *v3 = new Vector3f(v3_[0], v3_[1], v3_[2]);
    // if the magnitude is less than the margin of error, it passes
    ...
    vector< vector<Vector3f*> > triangle_verts;
...
void BezierPatch::uniform_subdivision(float step_size) {
    ...
    for(int i=0; i<=num_subdivisions; i++) {
        ...
        vector<Vector3f*> row(num_subdivisions+1);
...
Vector3f* BezierPatch::calculateUVpoint(float u, float v) {
...
Vector3f* BezierPatch::calculateUVnormal(float u, float v, float uv_delta) {
	Vector3f* p = calculateUVpoint(u, v);
	Vector3f* p_u = calculateUVpoint(u + uv_delta, v);
	Vector3f* p_v = calculateUVpoint(u, v + uv_delta);

	Vector3f* n = new Vector3f((*p_u - *p).cross(*p_v - *p));
...
BezierPatch::BezierPatch(vector<vector<Vector3f*> > a) {
    for(int i=0; i<4; i++)
    {
        vector<Vector3f*> row(4);
        p.push_back(row);
    ...
    if (draw_t == UNIFORM) {
        ...
        for(unsigned int i=0; i<uniform_p.size()-1; i++) {
            for(unsigned int j=0; j<uniform_p[i].size()-1; j++) {
				Vector3f a0 = T * (*(uniform_p[i+1][j]));
                Vector3f a1 = T * (*(uniform_p[i][j]));
                Vector3f a2 = T * (*(uniform_p[i][j+1]));
                Vector3f a3 = T * (*(uniform_p[i+1][j+1]));
        ...
        for(unsigned int i=0; i<adaptive_p.size(); i++) {
            // draw each triangle
            Vector3f a0 = T * (*(adaptive_p[i][0]));
            Vector3f a1 = T * (*(adaptive_p[i][1]));
            Vector3f a2 = T * (*(adaptive_p[i][2]));
...
vector<vector<Vector3f*> > BezierPatch::get_matrix() {
    return p;
}

void BezierPatch::set_matrix(vector<vector<Vector3f*> > a) {
    for(int i=0; i<4; i++) {
        for(int j=0; j<4; j++) {
            p[i][j] = a[i][j];
        }
